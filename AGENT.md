...
Hamlib、多电台、USB CAT、通用 RadioControl、Radio Audio、Icom LAN 泛化等长期规划已独立维护在 [ROADMAP.md](ROADMAP.md)。

**AGENT.md 不把 Roadmap 当作当前任务清单。** Agent 只有在用户明确要求推进某个阶段时，才按 Roadmap 实施；未明确要求时，不得因为路线图存在就自动开始下一阶段。

当前已完成的 Hamlib PR1/PR2 以及后续阶段的实施顺序、验收条件和架构边界，以 ROADMAP.md 为准；本文件只保留会直接约束当前代码修改的稳定工程不变量。

### 16.1 当前 Hamlib 边界

- Hamlib 负责电台控制语义；Android/Kotlin 负责设备、USB、网络 transport；APRSdroid 负责 APRS/AX.25/AFSK/DSP。
- Graywolf 继续作为生产本地 PCM AFSK1200 RX 的唯一 decoder。
- 现有 IC-705 WLAN backend、PTT 安全状态机和 session/recovery 语义不得因为 Hamlib 接入而降级或被首轮重写。
- 不为不同电台型号复制 APRS backend；型号差异优先进入通用 RadioControl/Hamlib 层。
- JNI 保持薄层，业务逻辑不迁入 native；同一 Hamlib rig handle 的调用必须串行化。
- 普通 Hamlib PTT 后端若无法提供 ACK/readback 级确认，UI/诊断必须明确区分“命令已提交”和“已确认 RX”；IC-705 WLAN 继续遵守本文件第 7 节更严格的 ACK/PTT 安全不变量。

历史路线、PR1～PR7 的详细设计和验收条件见 ROADMAP.md；历史完成细节见 CHANGELOG.md 与 Git 历史。
