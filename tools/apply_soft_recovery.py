...
session = "src/ic705/session/Ic705RxSession.kt"
controller = "src/ic705/backend/Ic705WifiBackendController.kt"
policy_test = "test/java/org/aprsdroid/app/ic705/session/Ic705WatchdogPolicyTest.kt"

if "streamRecoveryResponseMillis" in Path(session).read_text():
    print("soft recovery patch already applied")
    raise SystemExit(0)
...
old_watchdog = """    private fun checkWatchdog() {
...
new_watchdog = """    private fun checkWatchdog() {
        val now = monotonicMillis()
        val pttPossiblyAsserted = pttStateMachine.isTransmitting
...
"""
replace_once(session, old_watchdog, new_watchdog)
...
print("soft recovery patch applied")
