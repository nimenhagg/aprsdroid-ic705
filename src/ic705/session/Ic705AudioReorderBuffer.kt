...
internal class Ic705AudioReorderBuffer(
    ...
    private fun deliver(
        ...
        if (observedSampleCount in MIN_SANE_OBSERVED_SAMPLES..MAX_SANE_OBSERVED_SAMPLES) {
    ...
    private fun samplesForConcealment(sequence: Int): Int {
        ...
        return if (observed in MIN_SANE_OBSERVED_SAMPLES..MAX_SANE_OBSERVED_SAMPLES) {
            observed
        } else {
            ic705SamplesPerReceivePacket(sequence)
        }
    }

    companion object {
        /** Minimum non-runt sample count accepted for learning packet size (~0.08 ms). */
        const val MIN_SANE_OBSERVED_SAMPLES = 1
        /** Maximum sample count accepted for learning packet size (~40 ms @ 12 kHz). */
        const val MAX_SANE_OBSERVED_SAMPLES = 480
    }
}
