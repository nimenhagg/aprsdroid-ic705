 ...
 * Modified for APRSdroid: make generated R.jar discovery platform-neutral.
 *
...
//            }.files)
        ...
        project.tasks.register(genDeduplicateClassesTaskName(src$vaName), ScalaDeDuplicateClassesTask) { ScalaDeDuplicateClassesTask classesTask ->
            ...
            classesTask.onlyIf('Scala compiler produced classes to de-duplicate') {
                classesTask.inputDir.get().asFile.exists()
            }
        ...
        if (javaCompile) {
            ...
            project.tasks.withType(ScalaCompile).getByName(scalaTaskName) { ScalaCompile scalaCompile ->
                ...
                if (isTest) {
                    ...
                    if (processRes) {
                        final rJar = processRes.outputs.files.files.find {
                            it.name == 'R.jar' && it.parentFile?.name == variant.name
                        }
                        if (rJar == null) {
                            throw new GradleException("Cannot find ${variant.name} R.jar in ${processRes.name} outputs: ${processRes.outputs.files.files*.path}")
                        }
                        final files = project.files(rJar)
                        ...
                        final rJar = rFile.outputs.files.files.find {
                            it.name == 'R.jar' && it.parentFile?.name == variant.name
                        }
                        if (rJar == null) {
                            throw new GradleException("Cannot find ${variant.name} R.jar in ${rFile.name} outputs: ${rFile.outputs.files.files*.path}")
                        }
                        scalaCompile.classpath += project.files(rJar)
                    }
    ...
    private void evictCompileOutputForSrcTask(ScalaDeDuplicateClassesTask deduplicate, Project project, AbstractCompile dest, @Nullable Task src, @Nullable ScalroidExtension scalroid, String... srcDirs) {
        ...
        dest.doFirst {
            ...
            excludePaths.addAll(tempDirs.collect { path ->
                final platformPath = path.replace('/', File.separator).replace('\\', File.separator)
                platformPath.endsWith(File.separator) ? platformPath : (platformPath + File.separator)
            }.toSet().toList())
        ...
        dest.doLast {
            ...
            final allSrcMatchDirs = dest.source.files.findAll { file ->
                file.name.endsWith('.java') && excludePaths.any { file.path.indexOf(it, projLen) > 0 }
            }
            ...
            // A legacy Android project may keep Java files in a flat source directory even
            // though their package declarations are nested. Derive the generated class path
            // from the declaration instead of assuming that package == filesystem layout.
            final pkgOrNamesExcludes = allSrcMatchDirs.collect { file ->
                final className = file.name.substring(0, file.name.length() - '.java'.length())
                final packageMatcher = file.getText('UTF-8') =~ /(?m)^\s*package\s+([A-Za-z_$][A-Za-z0-9_$.]*)\s*;/
                final packageName = packageMatcher.find() ? packageMatcher.group(1) : ''
                packageName.isEmpty()
                        ? className
                        : packageName.tokenize('.').join(File.separator) + File.separator + className
            }
