...
* **IC-705 Wi-Fi 诊断页 Compose 重构（Jetpack Compose M3）**：
  - 彻底淘汰旧版 XML 布局，重构为全量 Material 3 风格声明式界面（`Ic705RxDiagnosticScreen`）。
  - 双通道连接参数卡片、实时数据仪表盘（音频块/PCM采样/AX.25帧/重置统计）、内存事件流（支持一键复制与自动滚动）。
* **签名方案升级（不再做 V1 签名）**：
  - 停用旧版 V1 (JAR) 签名，全面采用高效安全的 Android V2、V3、V4 签名体系。
* 版本元数据更新为 `1.8.5-ic705`（`versionCode 2026082585`）。
