## 🌟 [v1.2.1-ic705] - 2026-08-23

### 🚀 Target SDK 升级至 Android 17 (API 37)
* **API Level 37 (Android 17 Cinnamon Bun)**：正式将 `targetSdk` 升级至 API 37，全面接入 Android 17 全新系统容器架构。

### 📻 IC-705 Wi-Fi 诊断全面重构与设置归位
* **首选项直达入口**：在主设置「连接偏好设置」及「IC-705 连接配置」中内置了 **「IC-705 Wi-Fi 诊断」** 专属选项，彻底移出主界面右上角 3 点菜单；
* **参数全自动同步**：移除了繁琐易错的重复输入框，诊断页面自动从应用首选项中读取电台 IP、UDP 端口与认证凭据并结构化展示；
* **Material 3 卡片重构**：重构了诊断页面布局，提供大号 52dp 胶囊控制按键、实时通信遥测指标卡与深色终端日志，修复了顶部标题栏被压断问题。
