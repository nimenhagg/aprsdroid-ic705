...
### Added
- 主页台站列表新增呼号/备注搜索：忽略呼号大小写，过滤后保持距离排序与显示期限，支持清空和关闭搜索；匹配在 300 条结果限制之前完成。

### Fixed
- 修复 APRS-IS 密集收包时地图与整个主界面卡顿：地图使用内容相等的快照和稳定图标缓存，位图生成与 GeoJSON 序列化移出主线程，Google Marker 按呼号增量更新。
- 台站、地图和报文查询采用单查询与合并待处理请求；隐藏页面不再随每条报文全量查询，地图页只额外读取本台位置。
- 刷新协程交由 Activity 的 ViewModelStore 管理；过滤条件切换时丢弃过时查询结果，切换地图样式时隔离旧样式回调。

### Changed
- 版本更新为 `Mod-v2.2.4`（`versionCode 2026091301`）；同步中英文 README、维护规范和界面架构说明。

## [Mod-v2.2.3] - 2026-09-13

### Added
- 新增 APRS 1.1 Reply-ACK：支持 `{MM}AA` / `{MM}`，入站 piggyback ACK 可直接确认待发消息，普通 ACK 会保留并回显完整线号；重试发送时动态携带最新待回 ACK。
- Reply-ACK 新消息号限制在 1–99 循环，并跳过仍待确认的编号；升级前数据库中的旧长消息号保持原样发送，避免破坏兼容性。

### Fixed
- 修复 IC-705 PTT ON ACK 延迟时可能被误当成 PTT OFF ACK、导致软件假报 RX 的竞态；有歧义的释放现在会读取 CI-V PTT 状态，仍为 TX 时继续请求 PTT OFF。
- 修复 TNC2/TCP 对端 EOF 被转换为空字符串后可能持续空转的问题；EOF 现在进入现有 I/O 重连路径。
- 修复 javAPRSlib 将 `acknowledged{12`、`rejected ...{12` 等普通正文误判为 ACK/REJ 的问题；消息入口按 APRS 控制格式重新严格解析。
- 修复消息去重永久生效的问题：无编号重复消息不再被永久吞掉；编号消息的重复抑制限制在当前重试窗口内。
- 修复 `CALL` 与 `CALL-0` 在消息目标、ACK 匹配、会话历史与数据库查询中身份不一致的问题。
- 修复最后一次消息重试发送后，队列重扫可能丢失最终超时唤醒、使消息长期停留在待确认状态的问题。
- 配置导入改为后台流式读取并在读取过程中执行 256 KiB 上限；外部 URI 导入在写入前必须由用户确认。
- 配置导入使用显式 key→类型 schema，补齐当前后端/定位/地图设置与旧版 Boolean 类型，支持 `StringSet` 往返；运行时/未知键安全跳过，导出端只写可安全回导的设置。
- IC-705/AX.25 射频发送遇到无法无损编码的文本时明确拒绝，不再静默替换为 `?`；APRS-IS 文本路径保持不变。

### Changed
- 版本更新为 `Mod-v2.2.3`（`versionCode 2026091300`）。

> 注：本版本仍未改变 APRS-IS 既有 LoTW/实验性 TLS trust 行为。发布前使用 GitHub-hosted Ubuntu 实测：`ssl.aprs2.net` 可解析到 `44.25.16.4`，同一 runner 可连接 `china.aprs2.net:14580`，但 `ssl.aprs2.net:24580` 与直连 `44.25.16.4:24580` 均在 TLS 握手前超时，因此没有取得可用于证明系统 CA + hostname verification 兼容性的服务器证书结果；在可成功完成真实 24580 握手前不贸然替换 legacy trust 行为。
