...
### 📡 即时位置信标触发修复 & 消息会话即时渲染 & V1+V2+V3 签名 & 原生地图脱壳
* **消息会话发送即时上屏修复**：
  - 修复 `MessageActivity` 发送消息后未触发本地数据重载导致必须退出重进才能看到已发消息的 Bug；
  - 消息发送、重发、中止操作现已实现 0ms 立即上屏与自动滚动至底部。
* **现代重绘高清 APRS 电台图标**：
  - 升级为 2048x2304 Ultra HD 矢量高清图集，并在 `SymbolView` 中引入 Material You 动态取色圆角微卡片徽章容器。
