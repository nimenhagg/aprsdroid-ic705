...
(cd "$BLD" && AR="$AR" AS="$AS" CC="$CC" CXX="$CXX" LD="$LD" RANLIB="$RANLIB" STRIP="$STRIP" CFLAGS="-Os -fPIC -ffunction-sections -fdata-sections" CXXFLAGS="-Os -fPIC -ffunction-sections -fdata-sections" LDFLAGS="-Wl,-z,max-page-size=16384 -Wl,--gc-sections" "$SRC/configure" --host="$HOST" --prefix=/hamlib --without-libusb --disable-static --enable-shared --without-readline --without-cxx-binding --disable-winradio && make -j"$(getconf _NPROCESSORS_ONLN 2>/dev/null || echo 2)" V=0 --no-print-directory && make install DESTDIR="$INST" --no-print-directory)
